...
> - Before you proceed with the installation, please make sure to go through the [requirements](https://docs.deepsource.com/docs/enterprise-server-requirements) section of the documentation.

To install DeepSource Enterprise Server in the Standalone method, simply run the installation script in one of the nodes.

```Text Shell
curl -sSL [https://kurl.sh/deepsource-fowl](https://kurl.sh/deepsource-fowl) | sudo bash
```
