---
title: Vector
description: How to set up Vector to ship logs to Parseable on a Kubernetes cluster using Vector's kubernetes_logs source to collect logs from Kubernetes pods.
slug: how-to-setup-vector
visibility: PUBLIC
label: Vector
---

<head>
<title>How to Set Up Vector to Parseable | Parseable</title>
<meta charSet="utf-8" />
<meta name="twitter:card" content="summary" />
</head>

Vector is a lightweight, ultra-fast tool for building observability pipelines. Vector can be configured to send logs to Parseable with [HTTP Sink](https://vector.dev/docs/reference/configuration/sinks/http/) and JSON codec. 
...
We use the official [Vector Helm chart](https://github.com/vectordotdev/helm-charts). But, we'll use a modified `values.yaml` file, that contains the configuration for Fluent Bit to send logs to Parseable.
...
Let's take a deeper look at the Vector sink configuration in `values.yaml`. Important to notice here is that we use the sink type `http`. Source can be any of the [supported sources](https://vector.dev/docs/reference/configuration/sources/) by Vector.
