---
title: AWS ECS
description: Learn step-by-step how to set up Parseable on AWS ECS with Fargate. Streamline deployment and optimize performance effortlessly!
slug: setup-parseable-on-aws-ecs
visibility: PUBLIC
label: AWS ECS
---

<head>
<title>How to Setup Parseable on AWS ECS with Fargate? | Parseable</title>
<meta charSet="utf-8" />
...
AWS Fargate is a serverless technology that you can use with ECS to run containers without having to manage servers or clusters of Amazon EC2 instances. Read more about Fargate in the [AWS documentation](https://docs.aws.amazon.com/AmazonECS/latest/userguide/what-is-fargate.html).

## Prerequisites

- AWS account with access to ECS and Fargate.
- VPC and Security Group are configured to allow inbound traffic on port 80/443.
- Define the Task IAM role and set policies according to [Task IAM Roles](https://docs.aws.amazon.com/AmazonECS/latest/userguide/task-iam-roles.html)

Refer the ECS Prerequisites section of [AWS documentation](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/getting-started-fargate.html#first-run-prereqs) for specific details on VPC and Security Group configuration.
...
1. In the navigation pane, choose Clusters, and then select the cluster you created in the first step.
