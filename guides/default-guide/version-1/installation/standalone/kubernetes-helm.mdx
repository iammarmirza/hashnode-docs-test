...
description: Effortlessly deploy Parseable on Kubernetes via Helm in S3 or local mode. Simplify setup and optimize performance. Get started with ease!
slug: setup-parseable-on-kubernetes-via-helm
visibility: PUBLIC
label: Kubernetes Helm Chart
---

<head>
<title>How to Setup Parseable on Kubernetes via Helm? | Parseable</title>
<meta charSet="utf-8" />
...
You should now be able to point your browser to [http://localhost:8000](http://localhost:8000) and see the Parseable login page. You can login with the values set in the `username` and `password` fields in the `parseable-env-secret` file above.
...
MinIO installation steps above are for testing purposes only. For production, please refer to the [MinIO documentation](https://min.io/docs/minio/kubernetes/upstream/index.html).
...
You should now be able to point your browser to [http://localhost:8000](http://localhost:8000) and see the Parseable login page. You can login with the values set in `username` and `password` fieldx in the `parseable-env-secret` file above.
