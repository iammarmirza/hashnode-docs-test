...
description: Learn how to set up Redpanda, along with Redpanda Connect to send logs to Parseable.
slug: setup-redpanda-connector
visibility: PUBLIC
label: Redpanda Connector
---

<head>
<title>How to Set Up Redpanda Connector | Parseable</title>
<meta charSet="utf-8" />
...
This document explains how to set up Redpanda, along with [Redpanda Connect](https://github.com/redpanda-data/connect) to send logs to Parseable using Docker Compose as the deployment platform. We assume here that you have producer applications that are sending logs/events to Redpanda.

### Prerequisites

Please ensure Docker Compose is [installed on your machine](https://docs.docker.com/compose/install/).
...
You can now check the Parseable dashboard at [http://localhost:8000](http://localhost:8000), you should see this data in the dashboard.
