...
description: Discover common environment variables supported by Parseable Server. Optimize configurations for seamless integration and efficient log management.
...
<head>
<title>Environment Variables Supported by Parseable Server | Parseable</title>
<meta charSet="utf-8" />
...
| `P_PASSWORD` | Yes | Password for the admin user. Will be used to access API and UI.  | `admin` | `wJalrXUtna` |
| `RUST_LOG` | No | Control the log level of Parseable server. By default, all logging is disabled, except for the `error` level. [Refer the docs here](https://docs.rs/env_logger/0.10.1/env_logger/#enabling-logging) for possible values. | `error` | `info` |
...
| `P_OPENAI_API_KEY` | No | Specify your OpenAI API key to generate SQL automatically from plain text. [Read more here](/integrations/llm-based-sql-generation). | - | `sk-open-ai-api-key` |
...
| `P_HOT_TIER_DIR` | No | Path on the query node (or standalone node), where the Parseable server would store recent data. Refer [Hot Tier documentation for details](/features/tiering). | - | `/home/user/hot-tier` |
| `P_MAX_DISK_USAGE_PERCENT` | No | Maximum percentage of total disk that should be used for hot tier. Refer [Hot Tier documentation for details](/features/tiering). | `80` | `70` |
...
| `P_S3_PATH_STYLE` | No  | Force Parseable to use [Path style access to S3 store](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-bucket-intro.html#path-style-url-ex).  | true | false |
...
| Variable Name | Required | Description | Default | Example |
|-------|----|-------------------|----|-----------|
| P_OIDC_CLIENT_ID | No | Your OIDC client identifier, provided by your identity provider. | - | `client-id` 
| P_OIDC_CLIENT_SECRET | No | Your OIDC client secret, provided by your identity provider. | - | `client-secret` |
| P_OIDC_ISSUER | No | The OIDC issuer URL, typically provided by your identity provider. It points to the OIDC authorization server. Should support discovery protocol | - | `https://accounts.google.com` |
| P_ORIGIN_URI | No | The URI where Parseable is hosted or accessible. This should be the base URL of your Parseable instance. | - | `https://demo.parseable.com/` |

Refer to [OIDC section](https://www.parseable.com/docs/oidc) for more details.
